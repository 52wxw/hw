...
COLLECT_SERVICE_URL = os.getenv("COLLECT_SERVICE_URL", "http://collect-service:8001")
...
def execute_inspect_task(task_id: int, device_ids: list):
    ...
    auto_repair = task[0] if task else False
    
    # 获取设备信息
    placeholders = ','.join(['%s'] * len(device_ids))
    cursor.execute(f"SELECT id, ip, vendor, model, protocol, username, password_enc FROM device WHERE id IN ({placeholders})", device_ids)
    devices = cursor.fetchall()
    conn.close()
    
    # 遍历设备执行巡检
    for device in devices:
        device_id, ip, vendor, model, protocol, username, password_enc = device
        
        try:
            # 1. 调用采集服务
            collect_resp = requests.post(
                f"{COLLECT_SERVICE_URL}/api/collect",
                json={
                    "device_id": device_id,
                    "ip": ip,
                    "vendor": vendor,
                    ...
                    "protocol": protocol,
                    "username": username,
                    "password": password_enc  # 注意：实际应该解密
                },
                timeout=30
            )
            
            if collect_resp.status_code != 200:
                print(f"设备 {device_id} 采集失败：{collect_resp.text}")
                continue
            
            # 2. 调用AI分析服务
            ai_resp = requests.post(
                f"{AI_SERVICE_URL}/api/ai/analyze",
                json={
                    "device_id": device_id,
                    "scenario": "定时巡检",
            ...
            if ai_resp.status_code == 200:
                result = ai_resp.json()
                # 保存巡检报告到数据库
                save_inspect_report(device_id, result)
                print(f"设备 {device_id} 巡检完成，健康评分：{result.get('health_score', 0)}")
            else:
                print(f"设备 {device_id} AI分析失败：{ai_resp.text}")
