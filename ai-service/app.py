...
# 初始化区块链客户端
bc_client = BlockchainClient(os.getenv("BLOCKCHAIN_URL"))
...
def analyze():
    ...
    try:
                ...
                if fault.get("auto_repairable") and fault["level"] in ["P3", "P2"]:
                    try:
                        ...
                        repair_results.append({
                            ...
                            "success": "成功" in repair_result or "完成" in repair_result
                        })
                        # 如果修复成功，将解决方案存入知识库
                        if repair_results[-1]["success"]:
                            knowledge_base.add(
        ...
        log_data = {
            ...
            "report_hash": bc_client._calc_hash(report)
        }
        bc_client.upload_log(log_data, operator="ai-system")
